  ...
  )
{
  ...
  for (RPIndex = 1; RPIndex <= RootBridge->RootPortCount; RPIndex++) {
    ...
    if ((RootBridge->RootPort[RPIndex]->Device > 0) && (RootBridge->RootPort[RPIndex]->Device < 5)) {
      ...
      Status = AmdAmlCodeGenMethodInvokeMethodDsm (DeviceNode);
      if (EFI_ERROR (Status)) {
        ASSERT_EFI_ERROR (Status);
      }

      Status = AmdAmlCodeGenMethodInvokeMethodOst (DeviceNode);
      if (EFI_ERROR (Status)) {
        ASSERT_EFI_ERROR (Status);
      }
    }
  ...
  )
{
  ...
  for (Index = 0, RootBridge = RootBridgeHead; Index < RootBridgeCount; Index++, RootBridge++) {
    ...
    Status = AmdAmlCodeGenMethodInvokeMethodOsc (DeviceNode);
    if (EFI_ERROR (Status)) {
      ASSERT_EFI_ERROR (Status);
      return Status;
    }
  ...
  )
{
  ...
  for (RbIndex = 0, RootBridge = RootBridgeHead; RbIndex < RootBridgeCount; RbIndex++, RootBridge++) {
    ...
    Status = AmdAmlCodeGenMethodInvokeMethodOsc (PciNode);
    if (EFI_ERROR (Status)) {
      ASSERT_EFI_ERROR (Status);
      return Status;
    }
