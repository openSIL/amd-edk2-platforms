  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    BusLength = (UINTN)LShiftU64 (1, (UINTN)(RShiftU64 (AsmReadMsr64 (0xC0010058), 2) & 0xF));
    for (i = 0; i < SocPresent; i++) {
      for (j = 0; j < RbPerSocketPresent; j++) {
        FabricResourceForEachRb->PciBusNumber[i][j] = (UINT16)(BusLength / (SocPresent * RbPerSocketPresent));
      }
    }
